...
from snowflake.snowpark import DataFrame, CaseExpr
...
def create_region_dim(all_sales_df,session)-> None:
    ...
    region_dim_df = all_sales_df.groupBy(F.col("Country"),F.col("Region")).count()
    region_dim_df.show(2)
    region_dim_df = region_dim_df.with_column("isActive",F.lit('Y'))
...
def create_product_dim(all_sales_df,session)-> None:
    ...
    product_dim_df = all_sales_df.with_column("Brand",F.split(F.split('MOBILE_KEY'),F.lit('/'))[0])     \
                                .with_column("Model",F.split(F.split('MOBILE_KEY'),F.lit('/'))[1])      \
                                .with_column("Color",F.split(F.split('MOBILE_KEY'),F.lit('/'))[2])      \
                                .with_column("Memory",F.split(F.split('MOBILE_KEY'),F.lit('/'))[3])     \
                                .select(F.split('mobile_key'),F.split('Brand'),F.split('Model'),F.split('Color'),F.split('Memory'))
    
    product_dim_df = product_dim_df.select(F.col('mobile_key'),                    \
                                        F.cast(F.col('Brand'), F.StringType()).as_("Brand"),\
                                        F.cast(F.col('Model'), F.StringType()).as_("Model"),\
                                        F.cast(F.col('Color'), F.StringType()).as_("Color"),\
                                        F.cast(F.col('Memory'), F.StringType()).as_("Memory")\
                                        )
    
    product_dim_df = product_dim_df.groupBy(F.col('mobile_key'),F.col("Brand"),F.col("Model"),F.col("Color"),F.col("Memory")).count()
    product_dim_df = product_dim_df.with_column("isActive",F.lit('Y'))
...
def create_promocode_dim(all_sales_df,session)-> None:
    ...
    promo_code_dim_df = all_sales_df.with_column( "promotion_code", F.expr("case when promotion_code is null then 'NA' else promotion_code end"))
    promo_code_dim_df = promo_code_dim_df.groupBy(F.col("promotion_code"),F.col("country"),F.col("region")).count()
    promo_code_dim_df = promo_code_dim_df.with_column("isActive",F.lit('Y'))
...
def create_customer_dim(all_sales_df, session) -> None:
    customer_dim_df = all_sales_df.groupBy(F.col("COUNTRY"),F.col("REGION"),F.col("CUSTOMER_NAME"),F.col("CONCTACT_NO"),F.col("SHIPPING_ADDRESS")).count()
    customer_dim_df = customer_dim_df.with_column("isActive",F.lit('Y'))
...
def create_payment_dim(all_sales_df, session) -> None:
    payment_dim_df = all_sales_df.groupBy(F.col("COUNTRY"),F.col("REGION"),F.col("payment_method"),F.col("payment_provider")).count()
    payment_dim_df = payment_dim_df.with_column("isActive",F.lit('Y'))
...
def main():
    ...
    all_sales_df = all_sales_df.with_column( "promotion_code", F.expr("case when promotion_code is null then 'NA' else promotion_code end"))
